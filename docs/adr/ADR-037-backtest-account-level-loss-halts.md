...
issue #153 は H5 も同じ閾値で止まったと報告しているが、ノート自体には棄却理由や停止期間の記載がなく、本 ADR でも停止原因を独立に確認したものとは扱わない。
...
- issue #153 の報告に基づき、過去の研究 run（H4 の 2 腕、H5 を含む）は旧閾値による打ち切りを含む測定として扱い、本 ADR 以降の run と直接比較しない。比較する場合は両腕を修正後の同じ設定で流し直す。
