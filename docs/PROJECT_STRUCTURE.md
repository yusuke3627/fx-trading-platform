...
`strategies.<id>.instruments` 設定。base/quote 通貨 / pip size / contract size /
volume min / step / session / stop level / 受け付ける filling mode は
`InstrumentSpec` として Broker / Market Data Layer から取得する。
