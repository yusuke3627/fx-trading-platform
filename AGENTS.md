...
- コード変更を実装したら、**PR を作成する前に必ず現在の git 差分をレビューする**（実装後のセルフレビューでは指摘の提示で止まらず修正まで進める。ただし無関係な大規模リファクタはしない）
  - Claude Code・Codex ともにリポジトリ同梱の [`code-review-expert`](.agents/skills/code-review-expert/SKILL.md) スキルの手順と、本ファイル末尾「AIレビュー指示」のレビュー観点を併用する
  - 両エージェントとも上記の `SKILL.md` を読み、差分に該当する参照資料を必要時に読む。自動検出や個人環境へのインストールを前提にしない
  - スキルの手順で差分・呼び出し元・保存先・テストを確認し、プロジェクト固有の不変条件と重要度ラベルは本ファイルに従う
