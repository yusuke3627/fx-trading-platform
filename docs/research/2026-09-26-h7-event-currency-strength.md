...
- 判定（測定後に追記）: **止める。** `A` = −0.19、`C` = −0.25 で、どちらも関門の 0.10 に届かず、予言と逆の負だった。候補 3 はここで止め、確認期間のユーロドルは取り込まない。閾値・手順・期間は測定後に変えていない
...
2026-09-26 に 1 回だけ測った。report はタグ `prereg/h7-explore` のコミット `ef1a8d2` で、作業ツリーに差分がない状態
（`git_dirty` が false）で作った。report が記録した Plan・イベント一覧・tick の sha256 は、上の表と一致した。

### 件数

- 登録したイベント 79 件のうち、欠測（t0−1 時間 〜 t0+2 時間に tick の無い 1 時間がある）で 8 件を除いた
- 残る 71 件はすべてプラセボと対になった。プラセボの候補日も 12 日が欠測で除かれ、そのぶんは 1 週間後の候補を使った
- 約定に使う quote は 71 件すべてで見つかった

### 統計量（対になった 71 件、ブートストラップ 10,000 回、未定義 0 回）

| 統計量 | 点推定 | 片側 95% 下限 |
| --- | ---: | ---: |
| 継続 `C`（`m` の偏順位相関、`x` で条件付け） | −0.249 | −0.448 |
| 追加の予測力 `A`（`m` の偏順位相関、`r_UJ` で条件付け） | −0.189 | −0.386 |
| プラセボの `C` | −0.122 | −0.308 |
| プラセボの `A` | −0.126 | −0.319 |
| イベント − プラセボ の `C` の差 | −0.127 | −0.407 |
| イベント − プラセボ の `A` の差 | −0.063 | −0.326 |
| 対照: 先の値動きと `r_UJ` の順位相関 | −0.208 | （判定に使わないので区間は出していない） |

### 約定できる価格での損益（1 取引あたり、pips、71 件）

| 規則 | 平均 |
| --- | ---: |
| `m` の符号の向きへ建てる | −4.18 |
| `r_UJ` の符号の向きへ建てる（対照） | −3.65 |
| 差（`m` − `r_UJ`） | −0.53 |

### OANDA のスプレッド（確認期間のコスト規則の入力。判定には使わない）

| 時点 | 件数 | 中央値 | 75% | 90% |
| --- | ---: | ---: | ---: | ---: |
| t0+15 分 | 71 | 0.60 | 0.70 | 0.90 |
| t0+2 時間 | 71 | 0.60 | 0.70 | 0.80 |

確認へは進まないので、この値はコストの上乗せの計算には使われない。

## 判定

**止める。** 探索の関門は `A` と `C` がどちらも 0.10 以上だったが、`A` = −0.189、`C` = −0.249 で、どちらも予言と逆の負だった。
設計ノートの規則どおり、候補 3（イベント起点の通貨強弱）はここで止める。確認期間（2022-01〜2024-07-22）のユーロドルは取り込まず、
その期間の米国指標の窓は未使用のまま残る。

検出できる最小の偏相関は探索で 0.28〜0.30 と見込んでいた。今回の値は向きが逆なので、止める判断は検出力の不足によるものではない。

## 解釈（判定には使わない探索的な読み）

- この期間では、発表後 15 分のドル円の動きのうちドル全面の部分は、その後の 105 分で続くよりも戻る傾向にあった。ユーロ円の部分より
  強く戻った（`A` が負）。仕組みの節で予言した「ニュース由来の部分は注文フローで遅れて続く」は、この期間のこの時間幅では見えなかった
- プラセボ（同じ曜日・同じ 08:30 ET の非発表日）でも `A`・`C` は同じ向きに負で、大きさはイベントの半分（`C`）から 3 分の 2（`A`）ほどだった。戻る傾向の一部は、
  発表の有無によらない 08:30 ET の時間帯の性質と読める
- 対照の順位相関 −0.21 は、発表後 15 分の動きがその後の 105 分で一部戻ったことを示す。ただしこの期間は H5 が発表後のドル円で
  逆張りに近い戦略（失敗ブレイク）を測り、両腕とも純損失だった期間でもある。ここから「発表後の動きへの逆張り」を仮説に
  するなら、この結果を根拠にせず、事前に仕組みを示し、未使用の期間で事前登録してから測る必要がある。そうすると
  2022-01〜2024-07 の窓を使うことになる
