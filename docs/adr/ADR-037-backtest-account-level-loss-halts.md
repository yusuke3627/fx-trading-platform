...
その後 issue #155 で H4 / H5 の計 4 run の `risk_rejections` をコード単位で全件数え直した。
H5 の空白期間は銘柄あたりの建玉上限（`MAX_OPEN_POSITIONS_PER_SYMBOL`）によるもので、`HWM_DRAWDOWN_WITHIN_LIMIT` は確認ありが全期間で 0 件、確認なしが最終月 2026-08 の 99 件だけだった。
最大ドローダウンが 3% を超えていても、空白期間には HWM 停止が掛かっていない。
本 ADR が外す 3 つのうち `DAILY_LOSS_WITHIN_LIMIT` と `ROLLING_24H_LOSS_WITHIN_LIMIT` は 4 run すべてで 1 度も発火しておらず、実際に掛かったのは HWM だけである。
...
- 旧閾値による打ち切りを含むのは H4 の 2 腕で、有効側は 2025-05 以降の 15 か月（`HWM_DRAWDOWN_WITHIN_LIMIT` 6,376 件）、無効側は 2026-05 以降（同 1,450 件）が該当する。H5 の空白月には銘柄あたりの建玉上限の棄却が並び、口座水準の損失停止 3 種はいずれも記録が無いので、H5 の空白は旧閾値による打ち切りではない。いずれにせよ閾値が変わった以上、本 ADR より前の研究 run は H5 を含めて本 ADR 以降の run と直接比較しない。比較する場合は両腕を修正後の同じ設定で流し直す。
