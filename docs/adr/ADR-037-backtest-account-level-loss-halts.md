...
H5 の空白は HWM ドローダウン停止によるものではない。
ただし確認なしの最終月には棄却が 99 件あるため、旧閾値が H5 の標本に一切影響していないとまでは言えない。
daily と rolling 24h の 2 コードは再集計に含めていないので、本 ADR が外す 3 つ全部について否定したものではない。
...
- 旧 HWM 閾値による打ち切りを確認できたのは H4 の 2 腕で、有効側は 2025-05 以降の 15 か月、無効側は 2026-05 以降が該当する。H5 の空白月には銘柄あたりの建玉上限の棄却が並んでおり、HWM 停止の棄却は無い。daily と rolling 24h の 2 コードは H5 でも H4 でも集計していないので、旧閾値の影響がこの 2 つを含めて無かったとまでは言えない。いずれにせよ閾値が変わった以上、本 ADR より前の研究 run は H5 を含めて本 ADR 以降の run と直接比較しない。比較する場合は両腕を修正後の同じ設定で流し直す。
