...
1. **両腕とも 25 か月中 12 か月でしか約定しておらず、空白が 13 か月ある。** 空白は原因の違う 2 つに分かれる。2025-02〜2025-12 の 11 か月は、時間切れ決済が無い戦略なので、損切りに当たらない建玉が銘柄あたり 1 本の枠を占め続けたためと考えられる（H4 の無効側で銘柄別ポジション上限が支配的な棄却理由だったのと同じ構造）。加えて最大ドローダウンが 30,115 と 30,251 で、口座水準の 3% 停止（含み損益込みの資産が最高値から 3% 下がると新規建玉を承認しない規則）にも当たっている。残る 2026-02〜2026-03 の 2 か月は、市場記録そのものが無い。実行計画（`tasks/h5-macro-confirmation-ablation.md`）に記録したとおり、使った tick には 2026-01-23〜2026-04-08 の 74 日欠損があり、research engine は期間途中の欠損を検出せずに次の tick へ飛ぶ。
...
「維持したまま再測定」は、何を再測定するかで意味が変わる。seed は 2 種類ある。比較 CLI の `--seed` は bootstrap の再標本化に使う乱数系列を選ぶだけで、変えても CI の値が乱数誤差の範囲で揺れるだけで、精度は上がらず差そのものも動かない（精度を上げるのは `BOOTSTRAP_SAMPLES` を増やすこと）。一方、research の `--seed` は `ExecutionSimulator._shock_rng()` に入って reject・slippage・partial fill を決める。安定注文キーで共通注文のショックは腕をまたいで揃えてあるが、片腕にしか無い注文と、約定後の状態遷移には執行乱数が残る。**本ノートは research seed を 1 組（42）しか測っていない**ので、差の分散のうち執行由来がどれだけかは判定できていない。再測定するなら、対応した research seed を複数ペア流して執行感応度を確かめるのが先で、これは腕 2 本を並列で流すだけなので安い。標本を増やす意味での再測定は、時間を待って期間を延ばすより先に、訂正節のとおり空白だった 13 か月を測れる状態にすることに意味がある。うち 11 か月は研究リプレイの設定で取り戻せるが、2026-02〜2026-03 は tick そのものが無いのでバックフィルが要る。あわせて、決済のほぼ全部が protection fill になっている構造（利確・損切りの設計）を見直す。
...
- H5 は「差が検出できない」で閉じる。再測定を待つ理由は「データが増えるまで」ではなくなった。空白 13 か月のうち 11 か月は測定の側の問題で、両腕とも 12 か月で打ち切られていた。研究リプレイで口座水準の損失停止（日次・24 時間・最高値からのドローダウン）を掛けない設定と、保有が想定ホライズンを超えたら決済する時間切れ決済を両腕で有効にした状態で同じ期間を流し直せば、この 11 か月が戻って 23 か月の比較になる。残る 2026-02〜2026-03 は tick の 74 日欠損（2026-01-23〜2026-04-08）が原因なので、25 か月にするには tick のバックフィルも要る。再測定はこの 3 つが揃ってから VPS で行う。
