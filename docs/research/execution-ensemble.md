...
実効向上は波数ぶんには届かない。[H6 の事前登録](2026-09-17-h6-range-edge-reversal-preregistration.md)は `range_edge_reversal` の 4 腕を並列実行し 8 時間 2 分で完了した記録を残している。単独実行の換算（VPS 実測 6,855 tick/s、tick 100,358,953 本で約 4.1 時間）と比べると、4 並列での実効スループット向上は約 2 倍、1 本あたりの所要は約 2 倍に伸びた計算になる。ただし単独値は合成 tick からの換算で、当時の commit `b278551` は現在の main（非同期ループの統合と指標キャッシュを含む）と速度が違う。**並列度は小さい値から始め、実測して上げる。** live のティック収集や MT5 と同居するホストでは、`--max-parallel` をコア数より小さくする。研究対象の過去区間へのバックフィルは同時に実行しない。
...
Ctrl-C は検証が確定していない開始済みの試行を失敗として保存し、未開始の試行を起動しない。並列実行時は実行中の子プロセスを終了してから中断する。プロセス強制終了やホスト停止では最後の記録が `running` のまま残ることがあるが、全計画と不完全な状態は残る。保存に失敗した場合も、成功の終了コードは返さない。
