...
- 保有 1 秒で終わった約定が全腕に存在する（保有時間の最小値）。件数と原因は調べていない。損切り幅が entry 時点のスプレッドに対して狭すぎる可能性があるが、個別の約定を見ていないので断定できない。
  - 2026-09-22追記: [個別記録を確認](2026-09-22-h6-short-hold-diagnostic.md)した。実際は各腕共通の1件で1.262秒。判断時spread 0.3 pipsに対しstop距離24.9 pipsで、保存価格の急落によるSTOP_LOSSと整合した。上記は測定時点の未確認事項であり、H6の棄却は変えない。
