...
- 旧閾値による打ち切りを含むのは H4 の 2 腕で、有効側は 2025-05 以降の 15 か月、無効側は 2026-05 以降が該当する。H5 の空白は銘柄あたりの建玉上限によるもので、HWM ドローダウン停止による打ち切りではない（daily と rolling 24h は再集計の対象外）。ただし閾値が変わった以上、本 ADR より前の研究 run は H5 を含めて本 ADR 以降の run と直接比較しない。比較する場合は両腕を修正後の同じ設定で流し直す。
