...
`0011_execution_state_observations.sql`はcommandへ`state_revision`と`state_changed_at`を加え、`execution_state_observations`を作る。`transition`の注入Clock、Postgresのclaim時刻を保持し、insert・CAS更新・claimと観測を同じSQLで保存する。CAS失敗・観測保存失敗ではcommandだけが確定することはない。同一revisionの再保存は状態・数量・状態時刻が同一の場合だけ許し、異なる観測との競合はautocommitでもtransaction全体を取り消す。古いrevisionへの巻き戻しも拒否する。送信開始マーカー保存等は新しい状態遷移として数えない。競合は既存の`StaleCommandStateError`で呼出し元へ返し、READYへの復帰や注文の再送を行わない。
