...
2026-09-23 の Windows VPS 実測では、2026 年 7 月の `range_edge_reversal`（1 本 416 万 tick、commit `5405716`）の所要時間は次のとおりだった。4 並列の実効スループットは単独比 3.43 倍、1 本あたりの所要時間は約 16% 増えた。

| 並列度 | 1 本あたり | 全体 | 単独比の実効向上 |
| --- | --- | --- | --- |
| 単独 | 554 秒 | 559 秒 | 1.0 倍 |
| 2 並列 | 639 秒 | 647 秒 | 1.71 倍 |
| 4 並列 | 641 秒 | 646 秒 | 3.43 倍 |

同時に live のティック収集の取り込み遅延も悪化した。`received_at − event_time`（broker は UTC+3）の p99 は実験前 0.73 秒、4 並列中 11.5 秒、実験後 0.23 秒だった。研究リプレイと live 収集が同じ BelowNormal 優先度で CPU を取り合っていたため、Windows では研究の子を **Idle 優先度**で起動し、live 収集へ CPU を譲らせる。上表はこの対策前の実測であり、対策後の所要時間と取り込み遅延は Windows VPS で別途確認する。

**並列度は小さい値から始め、実測して上げる。** live のティック収集や MT5 と同居するホストでは、`--max-parallel` をコア数より小さくする。研究対象の過去区間へのバックフィルは同時に実行しない。
...
Ctrl-C は検証が確定していない開始済みの試行を失敗として保存し、未開始の試行を起動しない。実行中の子プロセスを終了してから中断する。Windows では逐次・並列とも子をジョブオブジェクトに入れ、親プロセス終了時に子も終了させる。`Stop-ScheduledTask` などの強制終了では後始末が走らず、`results.json` は最後の `running` のまま残るが、子はジョブオブジェクトで終了する。状態を保存して止めるには **Ctrl-C が望ましい**。ホスト停止でも記録が `running` のまま残ることがある。保存に失敗した場合は成功の終了コードを返さない。
